...
clean_edmonton_bp_columns <- function(bp, crs = NULL) {
  bp <- bp |>
    dplyr::select(
      -dplyr::any_of(c("count", "latitude", "longitude", "time_permi",
                       "time_issue", "row_id", "date_permi", "neighbourh",
                       "permit_num")),
  ...
  if (!is.null(crs)) bp <- sf::st_transform(bp, crs)
  bp
}
...
filter_edmonton_residential <- function(bp) {
  dplyr::filter(bp, .data$building_type %in% EDMONTON_RESIDENTIAL_BUILDING_TYPES)
}
...
add_edmonton_project_type <- function(bp) {
  bp |> dplyr::mutate(
    project_type = structure(
      forcats::fct(
        dplyr::case_when(
          .data$building_type %in% c("Backyard House (110)", "Garden Suite (110)") ~ "Backyard House",
          stringr::str_detect(.data$work_type, stringr::coll("new", ignore_case = TRUE)) &
            .data$units_added == 1 ~ "New SFH",
          .data$work_type %in% c(
            "(03) Interior Alterations", "(03) Exterior Alterations",
            "(07) Add Suites to Single Dwelling", "(08) Add Suites to Multi-Dwelling",
            "(09) Convert Non-Res to Residential", "(02) Addition",
          ...
          stringr::str_detect(.data$building_type, stringr::fixed("Row House", ignore_case = TRUE)) ~ "9+ Row House",
          .data$units_added >= 9 ~ "9+ Apartment",
          .default = NA_character_
        ),
        levels = c(
          "New SFH", "Addition/Conversion", "Backyard House",
          "Duplex to Fourplex", "Fiveplex to Eightplex",
          "9+ Row House", "9+ Apartment"
        )
      ),
      label = "Project type"
    )
...
add_edmonton_suite_info <- function(bp) {
  bp |> dplyr::mutate(
    ...
    has_backyard_home =
      stringr::str_detect(.data$job_description, stringr::regex("garden suites?", ignore_case = TRUE)) |
      stringr::str_detect(.data$job_description, stringr::regex("backyard houses?", ignore_case = TRUE))
  )
}
