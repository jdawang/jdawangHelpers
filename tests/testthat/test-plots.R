...
test_that("layers_frequent_bus_ecdf() returns a list", {
  result <- layers_frequent_bus_ecdf()
  expect_type(result, "list")
})

test_that("layers_frequent_bus_ecdf() with show_400m_line = TRUE returns 6 elements", {
  result <- layers_frequent_bus_ecdf(show_400m_line = TRUE)
  expect_length(result, 6)
})

test_that("layers_frequent_bus_ecdf() with show_400m_line = FALSE returns 4 elements", {
  result <- layers_frequent_bus_ecdf(show_400m_line = FALSE)
  expect_length(result, 4)
})
